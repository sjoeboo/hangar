...
- **WebUI: Progressive Web App (PWA) support** — the web UI can now be "installed" from Chrome
  (or any Chromium browser) as a standalone app window, keeping it out of your browser tab clutter.
  Uses `vite-plugin-pwa` with a Workbox service worker that precaches static assets for faster
  loads. API and WebSocket requests pass through to the network untouched. Includes app manifest
  with Hangar icons (192×192, 512×512), theme color matching the oasis_lagoon_dark palette, and
  `display: standalone` for a native-feeling window.
