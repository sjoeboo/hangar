...
func handleTry(profile string, args []string) {
	...
	fs.Usage = func() {
		fmt.Println("Usage: hangar try <name> [options]")
		fmt.Println()
		...
		fmt.Println("  hangar try redis-cache          # Create/find redis-cache experiment")
		fmt.Println("  hangar try rds                  # Fuzzy match 'redis-cache'")
		fmt.Println("  hangar try --list               # List all experiments")
		fmt.Println("  hangar try --list redis         # Fuzzy search experiments")
		fmt.Println("  hangar try myproject -c gemini  # Use Gemini instead of Claude")
		fmt.Println("  hangar try myproject --no-session  # Just create folder")
		fmt.Println()
